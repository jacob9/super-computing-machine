...
namespace Slave
{
    public class Cracking
    {
        ...
        public void RunCracking()
        {
            ...
            // Get password list.
            var userInfos = wsContext.GetPasswords();

            var result = new List<UserInfoClearText>();

            Console.WriteLine("Getting Dictionary...");

            // Instead of reading from an open filestream, the data is recieved in a list of string.
            var dictionary = wsContext.GetDictionary();
            // therefore, we simply do a foreach loop for each string element.
            if (!dictionary.Count.Equals(0)) // check that we actually have elements.
            {
                Console.WriteLine("Dictionary loaded succesfully! \nStarting Password-Cracking...");
        ...
        private IEnumerable<UserInfoClearText> CheckWordWithVariations(String dictionaryEntry, List<ClientUserInfo> userInfos)
        {
        ...
        /// <returns>A list of (username, readable password) pairs. The list might be empty</returns>
        private IEnumerable<UserInfoClearText> CheckSingleWord(IEnumerable<ClientUserInfo> userInfos, String possiblePassword)
        {
            ...
            foreach (ClientUserInfo userInfo in userInfos)
            {
